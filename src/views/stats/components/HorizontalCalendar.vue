<template>
  ...
  >
    <span class="left-arrow" @click="dateFlip(-1)">
      <slot name="leftIcon">
    ...
    <span class="right-arrow" @click="dateFlip(1)">
      <slot name="rightIcon">
        <svg class="icon" aria-hidden="true">
  ...
  methods: {
    init() {
      ...
      if (this.choosedDatePos === 'center') {
        let ts1 = firstDay.timestamp - parseInt(this.visibleDay / 2) * 1000 * 60 * 60 * 24
        this.firstDay = this.formatOneDay(ts1)
      } else if (this.choosedDatePos === 'right') {
        let ts2 = firstDay.timestamp - parseInt(this.visibleDay - 1) * 1000 * 60 * 60 * 24
    ...
    dateFlip(type) {
      ...
      if (type === -1) {
        ...
        let hasSpace = this.dateList.length * 50 - this.domWidth + this.translateX
      ...
      setTimeout(() => {
        ...
        if (beforeChangeX === afterChangeX) {
          ...
          this.firstDay = this.formatOneDay(
            this.formatDateTime(this.firstDay.timestamp - 3600 * 1000 * 24 * changeDay),
          )
        } else {
